...
function allSum() {
	for (var i = 0; i < allvalues.length; i++) {
		for (var j = 1; j < allvalues.length - (i + 1); j++) {
			...
			if (targetNumbers.indexOf(sum) === -1) targetNumbers.push(sum);
		}
	}
}
allSum();
function allProduct() {
	for (var i = 0; i < allvalues.length; i++) {
		for (var j = 1; j < allvalues.length - (i + 1); j++) {
			...
			if (targetNumbers.indexOf(product) === -1) targetNumbers.push(product);
		}
	}
}
allProduct();
...
function displayAttempt() {
	...
	if (attempt > 20) {
		for (var i = 0; i < 16; i++) {
			document.getElementById(i).style.pointerEvents = "none";
			document.getElementById("attempt").innerHTML = 20;
		}
		setTimeout(function () {
			endgame();
		}, 500);
